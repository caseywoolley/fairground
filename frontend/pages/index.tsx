import { Box, Button, Divider, Heading, Input, Link, ListItem, Text, UnorderedList, useToast } from '@chakra-ui/react'
...
const Home: NextPage = () => {
	...
	const CONTRACT_ADDRESS = isLocalChain ? LOCAL_CONTRACT_ADDRESS : GOERLI_CONTRACT_ADDRESS
			...
			toast({
				...
				description: (
					<>
						...
						<Text>
							<Link href={`https://goerli.etherscan.io/tx/${data?.blockHash}`} isExternal>
								View on Etherscan
							</Link>
						</Text>
					</>
	...
	async function fetchContractGreeting() {
		if (provider) {
			const contract = new ethers.Contract(CONTRACT_ADDRESS, YourContract.abi, provider) as YourContractType
			try {
		...
		<Layout>
			...
			<Box maxWidth='container.sm' p='8' mt='8' bg='gray.100'>
				...
				<Box>
					...
					<Button mt='2' colorScheme='teal' disabled={!address} onClick={fetchContractGreeting}>
				...
				<Box>
					...
					/>
					<Button mt='2' colorScheme='teal' isLoading={isLoading} disabled={!address || isLoading} onClick={() => write?.()}>
